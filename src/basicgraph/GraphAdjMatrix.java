...
//		List<Integer> list = getNeighbors(v);
//		for(Integer i: list) {
//		    result.addAll(getNeighbors(i));
//        }
//        return result;
	...
	private int[][] multiply(int[][] a, int[][] b) {
	    ...
	    int collumsInB = b[0].length;
        int collumsInA = a[0].length;
	    int[][] c = new int[rowsInA][collumsInB];
	    for(int i = 0; i < rowsInA; i++) {
	        for(int j = 0; j < collumsInB; j++) {
	            for(int k = 0; k < collumsInA; k++) {
	                c[i][j] += a[i][k] * b[k][j];
                }
            }
        }
